...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Jumpscares':
						ClientPrefs.jumpScare = !ClientPrefs.jumpScare;
						trace ('are jumpscares on? ' + FlxG.save.data.jumpScare);
					...
					case 'New vocals system (broken)':
						ClientPrefs.newVocals = !ClientPrefs.newVocals;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'New vocals system (broken)':
				daText = "IT *SAYS* that it's broken, but it works. I just lack the audio files lol.";
			case 'Ghost Tapping':
			...
			case 'Flashing Lights':
				daText = "Uncheck this if you're sensitive to flashing lights or jumpscares!";
			case 'Jumpscares':
				daText = "Uncheck this if you don't like jumpscares!";
			case 'Camera Zooms':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Jumpscares':
						daValue = ClientPrefs.jumpScare;
					case 'Downscroll':
						daValue = ClientPrefs.downScroll;
					case 'Middlescroll':
						daValue = ClientPrefs.middleScroll;
					case 'New vocals system (broken)':
						daValue = ClientPrefs.newVocals;
					case 'Ghost Tapping':
