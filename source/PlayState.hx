...
//import flixel.WebmIo;
//import flixel.WebmPlayer;
...
{
	//NEW SHIT
	public var isCrt:Bool = false;

	public var isHeying:Bool = false;
	public var isBFhurt:Bool = false;

	public var isStatic:Bool = false;

	public static var isSus:Bool = false;

	public var jsAnimeshoun : FlxSprite;
	public static var osuOverlay : FlxSprite;
	public static var osuJudgment : FlxSprite;
	public static var crtFilter : FlxSprite;
	public static var camOn : FlxSprite;
	public static var camOff : FlxSprite;
	public static var whiteAnimeshoun : FlxSprite;
	public var camAnimeshoun : FlxSprite;
	public var staticAnimeshoun:FlxSprite;
	public var nofriends:FlxSprite;
	public static var deathCause:String = Lang.ukTxt;
	public var lastWas:String = 'null';

	public var sus:FlxSprite;
	public var bfSolo:FlxSprite;
	public var lookAway:FlxSprite;

	public var pageAnima:FlxSprite;

	public var pageGot:FlxText;

	public var pageHave:Int = 0;
	public var gettingPage:Int;
	public var canGetPage:Bool = false;
	public var pageGotten:Bool = false;
	public var pageMisses:Int = 0;
	public var poisonMiss:Int = 0;

	public var willIdleBeats:Int = 0;

	public var isPresentation:Bool = false;

	public var slenderTrail:FlxTrail;
	//END OF NEW SHIT
	...
	public var enemyVocals:FlxSound;
	...
	override public function create()
	{
		...
		//WOODS OVERLAY
		pageAnima = new FlxSprite();
		pageAnima.frames = Paths.getSparrowAtlas('statics/page');
		pageAnima.animation.addByPrefix('idle', 'idle', 24, true);
		pageAnima.screenCenter();
		pageAnima.scrollFactor.set();
		pageAnima.antialiasing = false;
		pageAnima.animation.play('idle', true);
		pageAnima.visible = false;
		pageAnima.x = 1100;

		bfSolo = new FlxSprite().loadGraphic(Paths.image('blankBeeg'));
		bfSolo.scrollFactor.set();
		bfSolo.antialiasing = false;
		bfSolo.color = FlxColor.BLACK;
		bfSolo.alpha = 0;
		bfSolo.screenCenter();

		lookAway = new FlxSprite().loadGraphic(Paths.image('blank'));
		lookAway.scrollFactor.set();
		lookAway.antialiasing = ClientPrefs.globalAntialiasing;
		lookAway.color = FlxColor.BLACK;
		lookAway.visible = false;
		//lookAway.cameras = [camHUD];
		lookAway.screenCenter();

		crtFilter = new FlxSprite().loadGraphic(Paths.image('statics/crt'));
		crtFilter.scrollFactor.set();
		crtFilter.antialiasing = ClientPrefs.globalAntialiasing;
		crtFilter.screenCenter();

		sus = new FlxSprite().loadGraphic(Paths.image('statics/sus_flashing_red'));
		sus.scrollFactor.set();
		sus.color = FlxColor.RED;
		sus.visible = false;
		sus.alpha = 0.08;
		sus.screenCenter();

		whiteAnimeshoun = new FlxSprite();
		whiteAnimeshoun.frames = Paths.getSparrowAtlas('statics/white_scanline');
		whiteAnimeshoun.animation.addByPrefix('idle', 'scanline', 34, true);
		whiteAnimeshoun.screenCenter();
		whiteAnimeshoun.scrollFactor.set();
		whiteAnimeshoun.antialiasing = ClientPrefs.globalAntialiasing;
		whiteAnimeshoun.flipY = ClientPrefs.downScroll;
		whiteAnimeshoun.animation.play('idle');

		camOn = new FlxSprite().loadGraphic(Paths.image('statics/cam_on'));
		camOn.scrollFactor.set();
		camOn.antialiasing = ClientPrefs.globalAntialiasing;
		camOn.visible = false;
		camOn.screenCenter();

		camOff = new FlxSprite().loadGraphic(Paths.image('statics/cam_off'));
		camOff.scrollFactor.set();
		camOff.antialiasing = ClientPrefs.globalAntialiasing;
		camOff.screenCenter();

		jsAnimeshoun = new FlxSprite();
		jsAnimeshoun.frames = Paths.getSparrowAtlas('statics/jumpscare');
		jsAnimeshoun.animation.addByPrefix('idle', 'idle', 24, true);
		jsAnimeshoun.animation.addByPrefix('boo', 'jumpscare', 24, true);
		//jsAnimeshoun.screenCenter();
		jsAnimeshoun.scrollFactor.set();
		jsAnimeshoun.antialiasing = false;
		jsAnimeshoun.visible = false;
		jsAnimeshoun.animation.play('boo');

		staticAnimeshoun = new FlxSprite();
		staticAnimeshoun.frames = Paths.getSparrowAtlas('statics/static');
		...
		staticAnimeshoun.alpha = 0.1;
		staticAnimeshoun.visible = false;

		nofriends = new FlxSprite().loadGraphic(Paths.image('statics/nofriends'));
		nofriends.scrollFactor.set();
		nofriends.antialiasing = ClientPrefs.globalAntialiasing;
		nofriends.screenCenter();
		nofriends.visible = false;
		//WOODS OVERLAY END

		switch (SONG.song.toLowerCase())
		{
			case 'thanatophobia' | 'static' | 'haphephobia' | 'engage':
				curStage = 'woods';
				defaultCamZoom = 0.8;
				var FG : FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('slender/woods/ground'));
				var BG : FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('slender/woods/sky'));
				var lowBG : FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('slender/woods/low quality'));
				BG.screenCenter();
				FG.screenCenter();
				lowBG.screenCenter();
				BG.scrollFactor.set();
			...
			case 'arachnophobia':
				curStage = 'woods2';
				defaultCamZoom = 0.35;
				var FG : FlxSprite = new FlxSprite().loadGraphic(Paths.image('slender/woods2/ground'));
				var BG : FlxSprite = new FlxSprite().loadGraphic(Paths.image('slender/woods2/sky'));
				BG.screenCenter();
				FG.screenCenter();
				BG.scrollFactor.set();
				isCrt = true;
		...
		switch(curStage)
		{
			...
			case 'woods2':
				slenderTrail = new FlxTrail(dad, null, 10, 40, 0.3, 0.069);
				add(slenderTrail);
		}

		add(gfGroup);
	
		/*if (SONG.player2 == 'slenderman')
			gf.playAnim('scared');*/
		...
		if (curStage == 'woods2' && ClientPrefs.flashing == true) {
			add(bfSolo); }
		...
		if (ClientPrefs.flashing == true)
		{
			switch (SONG.song.toLowerCase())
			{
				case 'thanatophobia' | 'static' | 'haphephobia' | 'engage' | 'arachnophobia':
		...
		//lookAway.cameras = [camHUD];
		staticAnimeshoun.cameras = [camHUD];
		camOn.cameras = [camHUD];
		camOff.cameras = [camHUD];
		jsAnimeshoun.cameras = [camHUD];
		whiteAnimeshoun.cameras = [camHUD];
		crtFilter.cameras = [camHUD];
		pageAnima.cameras = [camHUD];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				case 'thanatophobia':
					...
					var cutBf:FlxSprite = new FlxSprite();
					cutBf.frames = Paths.getSparrowAtlas('cutscenes/thanato/bf');
					cutBf.animation.addByPrefix('idle', 'bf', 30, true);
					cutBf.x = boyfriend.x - 400;
					cutBf.y = boyfriend.y - 270;
					cutBf.scrollFactor.set();
					cutBf.antialiasing = ClientPrefs.globalAntialiasing;
					cutBf.animation.play('idle');

					var cutGf:FlxSprite = new FlxSprite();
					cutGf.frames = Paths.getSparrowAtlas('cutscenes/thanato/gf');
					cutGf.animation.addByPrefix('idle', 'gf', 30, true);
					cutGf.x = gf.x - 400;
					cutGf.y = gf.y - 200;
					cutGf.scrollFactor.set();
					cutGf.antialiasing = ClientPrefs.globalAntialiasing;
					cutGf.animation.play('idle');

					// add gay shit
					add(cutGf);
					add(cutBf);

					FlxG.sound.play(Paths.sound('cutscenes/one'));
					
					new FlxTimer().start(21.1, function(swagTimer:FlxTimer) {
						FlxG.camera.zoom = defaultCamZoom;
						cutBf.kill();
						cutGf.kill();
						camHUD.visible = true;
		...
		CoolUtil.precacheSound('pew');
		CoolUtil.precacheSound('pages/grab');
		CoolUtil.precacheSound('pages/spawn');
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					/*if (SONG.song.toLowerCase() == 'thanatophobia')
						{
							add(senpaiEvil);
							senpaiEvil.alpha = 0;
							new FlxTimer().start(0.3, function(swagTimer:FlxTimer)
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			{
				if (tmr.loopsLeft % gfSpeed == 0 && !gf.animation.curAnim.name.startsWith('sing'))
				{
					/*if (SONG.player2 == 'slenderman')
						gf.playAnim('scared');
					else
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices){
			...
			enemyVocals = new FlxSound().loadEmbedded(Paths.voices(PlayState.SONG.song));}
		else{
			vocals = new FlxSound();
			enemyVocals = new FlxSound();}

		FlxG.sound.list.add(vocals);
		FlxG.sound.list.add(enemyVocals);
	...
	function enemyResyncVocals():Void
	{
		if(finishTimer != null) return;
		...
		FlxG.sound.music.play();
		Conductor.songPosition = FlxG.sound.music.time;
		enemyVocals.time = Conductor.songPosition;
		if (ClientPrefs.newVocals == true)
			enemyVocals.play();
	}
	...
	override public function update(elapsed:Float)
	{
		...
		health -= 0.0001 * poisonMiss;

		if (FlxG.keys.pressed.CONTROL && FlxG.keys.justPressed.F12 || FlxG.keys.justPressed.CONTROL && FlxG.keys.pressed.F12){ //presentation mode
				isPresentation = !isPresentation;
				camHUD.visible = !camHUD.visible;
				if (curStage == 'woods2'){
					dad.visible = !dad.visible;
					slenderTrail.visible = !slenderTrail.visible;}}

		callOnLuas('onUpdate', [elapsed]);

		if (FlxG.keys.justPressed.SPACE) //take flying page
			{
				getPage();
			}
		/*if (FlxG.keys.justReleased.SPACE) //look ahead
			{
				lookAway.visible = false;
			}*/
		...
		if(ratingString == '?') {
			scoreTxt.text = Lang.scoreTxt + ' ' + songScore + ' | ' + Lang.missedTxt + ' ' + songMisses;
		} else {
			scoreTxt.text =  Lang.scoreTxt + ' ' + songScore + ' | ' + Lang.missedTxt + ' ' + songMisses + ' | ' + Lang.accTxt + ' ' + Math.floor(ratingPercent * 100) + '%';
		}
						...
						if (daNote.tooLate || !daNote.wasGoodHit)
						{
							if(!endingSong) {
								...
								switch(daNote.noteType) {
									...
									case 4: //distorted step
										//poisoned();
										health -= 0.5;
										killBF();
										deathCause = Lang.distDeath;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, ?onLua:Bool = false) {
		switch(eventName) {
			...
			case 'Toggle Jumpscare':
				if (jsAnimeshoun.visible == false)
					jsAnimeshoun.visible = true;
				else
					jsAnimeshoun.visible = false;
			...
			case 'BF Solo':
				trace ("BF solo started at beat " + curBeat);

				FlxTween.tween(FlxG.camera, {zoom: 0.8}, 36.4);
				FlxTween.tween(bfSolo, {alpha: 1}, 4);
				FlxTween.tween(sus, {alpha: 0}, 4);
				FlxTween.tween(camOff, {alpha: 0}, 4);
				FlxTween.tween(camOn, {alpha: 0}, 4);
				FlxTween.tween(whiteAnimeshoun, {alpha: 0}, 4);
				FlxTween.tween(crtFilter, {alpha: 0}, 4);
				//lookAway.alpha = 0;
			...
			case "BF Solon't":
				trace ("BF solo finished at beat " + curBeat);

				FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom}, 0.5);
				FlxTween.tween(bfSolo, {alpha: 0}, 0.5);
				FlxTween.tween(sus, {alpha: 0.08}, 0.5);
				FlxTween.tween(camOff, {alpha: 1}, 0.5);
				FlxTween.tween(camOn, {alpha: 1}, 0.5);
				FlxTween.tween(whiteAnimeshoun, {alpha: 1}, 0.5);
				FlxTween.tween(crtFilter, {alpha: 1}, 0.5);
				//lookAway.alpha = 1;
			...
			case 'Pico Shoot':
				FlxG.sound.play(Paths.sound('pew'));
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		else if (noteDiff > Conductor.safeZoneOffset * 0.75)
		{
			...
			score = 100;
			lastWas = daRating;
			//deathCause = Lang.badDeath;

			if (storyDifficulty == 2 || storyDifficulty == 1)
				killBF();
		}
	...
	override function beatHit()
	{
		...
		camOn.visible = !camOn.visible;

		if (sus.visible == true && curStage == 'woods2')
			sus.visible = false;
		else if (sus.visible == false && curStage == 'woods2')
			sus.visible = true;
	...
	public function killBFregardless():Void
	{
		var ret:Dynamic = callOnLuas('onGameOver', []);
		if(ret != FunkinLua.Function_Stop) {
			boyfriend.stunned = true;
	...
	public function spawnPage():Void
	{
		pageAnima.x = 1100;
		canGetPage = true;
		pageAnima.visible = true;
		FlxG.sound.play(Paths.sound('pages/spawn'));
		FlxTween.tween(pageAnima, {x: -500}, 2);
	...
	public function getPage():Void
	{
		...
		{
			pageAnima.visible = false;
			FlxG.sound.play(Paths.sound('pages/grab'));
			//FlxG.sound.play(Paths.sound('shoot'));
			canGetPage = false;
			pageHave ++;
			pageGot = new FlxText(0, 0, 0, pageHave + '/8 pages', 32);
			pageGot.screenCenter();
			pageGot.cameras = [camHUD];
			add(pageGot);
			new FlxTimer().start(3, function(swagTimer:FlxTimer)
			{
				FlxTween.tween(pageGot, {alpha: 0}, 1);
	...
	public function poisoned():Void
	{
		//iconP1.color = FlxColor.fromRGB(180, 50, 255);
		iconP1.changeIcon('poisonbf');
		boyfriend.color = FlxColor.fromRGB(180, 50, 255);
		poisonMiss++;
	}

	public function canAltIdle():Void
	{
		willIdleBeats++;
		if (willIdleBeats == 2)
		{
			var willStance:Int = FlxG.random.int(1, 10);
			if (willStance == 1)
				dad.playAnim('stance');
			else
				dad.playAnim('idle');
