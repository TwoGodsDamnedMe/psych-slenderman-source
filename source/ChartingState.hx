...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Change Scroll Speed', "Adds value 1 to scroll speed\nSubtracts value 2 from scroll speed\n\nBlank = 0"],
		['Toggle Static', "Toggle the static overlay"],
		['Toggle Jumpscare', "Toggle the jumpscare"],
		['Switch Stage to Static', "Switch stage to woods and back"],
		['Scared BF', "Activare FUCKING TERRIFIED BF HOLY FU-"],
		['Beaten Engage', "u beat engage.\npoggers."],
		['Good Luck', "Toggle Good Luck message"],
		['BF Solo', "That one part in Arachnophobia"],
		["BF Solon't", "End BF's solo"],
		['Spawn Page', "Spawn a page"],
		['Pico Shoot', "pico shoots slendy lol"]
		//['Speeen', "weeeeeeeeeee"]
		//['Among Us Toggle', "Toggle the flashing red lights"]
	];
	...
	**/
	...
	//	var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x, check_mute_inst.y + 30, null, null, "Mute Enemy Vocals (in editor)", 100);

		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x, check_mute_inst.y + 30, null, null, "Mute BF Vocals    (in editor)", 100);
		...
		var check_mute_enemyvocals = new FlxUICheckBox(check_mute_inst.x + 130, check_mute_inst.y + 30, null, null, "Mute Enemy Vocals (in editor)", 100);
		check_mute_enemyvocals.checked = false;
		check_mute_enemyvocals.callback = function()
		{
			if(enemyvocals != null) {
				var vol:Float = 1;

				if (check_mute_enemyvocals.checked)
					vol = 0;

				enemyvocals.volume = vol;
			}
		};
		...
		tab_group_chart.add(check_mute_enemyvocals);
	...
	function loadSong():Void
	{
		...
		var enfile:String = Paths.enemyvoices(currentSongName);
		...
		if (OpenFlAssets.exists(enfile)) {
			enemyvocals.loadEmbedded(enfile);
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if(enemyvocals != null) enemyvocals.stop();
			...
			if (FlxG.keys.justPressed.SPACE)
			{
				...
				else
				{
					...
					if(enemyvocals != null) {
						enemyvocals.play();
						enemyvocals.pause();
						enemyvocals.time = FlxG.sound.music.time;
						enemyvocals.play();
					}
